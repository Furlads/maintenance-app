            ...
            return (
              <button
                ...
                style={{
                  padding: "8px 12px",
                  borderRadius: 10,
                  border: assignedTo === workerValue ? "2px solid #000" : "1px solid #ddd",
                  background: "#fff",
      ...
      {jobs.map((j) => (
        <div key={j.id} style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.75 }}>
            {j.visitDate ? `Visit: ${toGBDate(new Date(j.visitDate))}` : `Visit: UNSCHEDULED`} • Assigned:{" "}
            {j.assignedTo ?? "none"} • Status: {j.status}
          </div>
        </div>
      ))}
    </main>
