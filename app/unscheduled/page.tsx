...
export default function UnscheduledPage() {
  ...
  // Default scheduling settings (can be overridden per job)
  const [defaultWorker, setDefaultWorker] = useState("stephen");

  // Customer insists fields
  ...
  // Per-job override worker (only used if you change it)
  ...
  function resolveWorkerForJob(job: Job) {
    // ✅ If job already assigned, use it.
    const existing = cleanLower(job.assignedTo);
    if (existing) return existing;

    // ✅ If user typed an override worker, use it.
    const over = cleanLower(overrideWorker);
    if (over) return over;

    // ✅ Otherwise use default picker
  ...
  async function scheduleEconomically(job: Job) {
    ...
    // Make job flexible + unscheduled, then rebuild will place it economically
  ...
  async function customerInsists(job: Job) {
    ...
    // Anchor set → rebuild everything else around it
    await rebuildDiary(w);
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {WORKERS.map((w) => (
              <button
                key={w}
                type="button"
                onClick={() => setDefaultWorker(w)}
                style={{
                  padding: "10px 12px",
                  borderRadius: 12,
                  border: defaultWorker === w ? "2px solid #000" : "1px solid #ddd",
                  background: "#fff",
            ...
            >
              <div
                ...
                onClick={() => {
                  ...
                  setOverrideWorker(""); // reset per-job override when opening
                }}
              >
