...
import WorkerSiteActions from './WorkerSiteActions'
...
function currentProgrammeDay(start: Date | null, totalDays: number) {
  if (!start || totalDays <= 0) return null
  const today = new Date()
  const startDay = new Date(start.getFullYear(), start.getMonth(), start.getDate())
  const todayDay = new Date(today.getFullYear(), today.getMonth(), today.getDate())
  if (todayDay < startDay) return null

  let cursor = new Date(startDay)
  let workingDay = 0
  while (cursor <= todayDay) {
    const day = cursor.getDay()
    if (day !== 0 && day !== 6) workingDay += 1
    cursor.setDate(cursor.getDate() + 1)
  }

  return Math.min(Math.max(workingDay, 1), Math.ceil(totalDays))
}
...
export default async function LandscapingWorkerJobPage({ params }: PageProps) {
  ...
  const todayDay = plan ? currentProgrammeDay(job.visitDate, plan.totalDays) : null
  const todayPlan = plan && todayDay ? plan.dayPlan.find((day) => day.day === todayDay) || null : null
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        <section className="rounded-3xl bg-zinc-950 p-5 text-white shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="text-xs font-black uppercase tracking-[0.18em] text-yellow-300">Landscaping job sheet</div>
              <h1 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">{job.customer.name}</h1>
              <p className="mt-2 text-sm leading-6 text-zinc-300">{job.address || job.customer.address || job.customer.postcode || 'Address not saved'}</p>
            </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-white/10 p-4">
              ...
              <div className="mt-1 text-xl font-black">{plan ? `${plan.totalDays} working day${plan.totalDays === 1 ? '' : 's'}` : 'Being prepared'}</div>
            </div>
            <div className="rounded-2xl bg-white/10 p-4">
              <div className="text-[11px] font-bold uppercase tracking-wide text-zinc-400">Team</div>
              <div className="mt-1 text-xl font-black">{assignedWorkers.length ? assignedWorkers.join(', ') : plan ? `${plan.teamSize} people planned` : 'Not assigned'}</div>
            </div>
            <div className="rounded-2xl bg-white/10 p-4">
        ...
        <section className="rounded-3xl border border-yellow-300 bg-yellow-50 p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-zinc-800">
            <div className="rounded-2xl bg-white p-4 ring-1 ring-inset ring-yellow-200"><strong>📐 Lines, levels and finish:</strong> Use string lines, profiles, levels and square checks properly. Do not eyeball work or cut corners to save time. Measure, check and build it to the agreed Furlads standard. If site conditions make the specification impossible, stop and speak to Trev/Kelly before changing the method.</div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-inset ring-yellow-200"><strong>🪵 Boards / ground protection:</strong> Put boards or suitable ground-protection sheets down before spoil or waste is stacked ready for the grabber. Once the waste has gone, move/reuse the boards under the mixer, cement, sand, paving and other materials so the customer’s drive, paths and garden are kept clean and protected.</div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-inset ring-yellow-200"><strong>⚡ Get ahead — don’t bank an early finish:</strong> Finishing today’s target early does not mean finishing early later in the week. If it is safe and the next stage is ready, use the time to pull work forward: set out the next area, move materials, make cuts, prep edges, compact, tidy waste, clean down, organise tools or complete snagging. A strong day should create headroom for weather, delays and problems later in the job.</div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-inset ring-yellow-200"><strong>✅ Before leaving:</strong> Leave the job safe, tidy and ready to restart quickly. Check the day target, photos, materials, tools and next-day setup before packing up.</div>
          </div>
        </section>
        ...
        {!plan ? (
          <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-900">The job has been accepted, but the day-by-day landscaping plan has not been generated yet. Kelly or Trev can regenerate it from the internal job planning page.</p>
            <div className="mt-4 rounded-2xl bg-white p-4 text-sm leading-6 text-zinc-800 ring-1 ring-inset ring-amber-200"><strong>Accepted scope:</strong><br />{workerSafeText(job.title)}</div>
          </section>
        ) : (
          <>
            {todayPlan ? (
              <section className="rounded-3xl border border-yellow-300 bg-yellow-100 p-5 shadow-sm">
                <div className="text-xs font-black uppercase tracking-[0.16em] text-yellow-800">Today’s focus — Day {todayPlan.day}</div>
                <h2 className="mt-1 text-2xl font-black text-zinc-950">{workerSafeText(todayPlan.heading)}</h2>
                <p className="mt-2 text-sm leading-6 text-zinc-800">{workerSafeText(todayPlan.target)}</p>
                <div className="mt-3 text-sm font-bold text-zinc-800">Get this stage right first. If you’re ahead, use the safe next-stage work in the programme below to move the whole job forward.</div>
              </section>
            ) : null}
              ...
              <div className="mt-5 border-t border-zinc-200 pt-4">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {plan.dayPlan.map((day) => (
                    <div key={`summary-${day.day}`} className={`flex items-center gap-3 rounded-2xl px-3 py-3 ${day.day === todayDay ? 'bg-yellow-100 ring-1 ring-inset ring-yellow-300' : 'bg-zinc-50'}`}>
                      <div className="flex h-9 w-9 flex-none items-center justify-center rounded-xl bg-yellow-300 text-sm font-black text-zinc-950">{day.day}</div>
                      <div className="min-w-0">
                        <div className="text-[10px] font-black uppercase tracking-wide text-zinc-500">Day {day.day}{day.day === todayDay ? ' — Today' : ''}</div>
            ...
            {controls.customerExtras.length || controls.extraItems.length ? (
              <section className="rounded-3xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm leading-6 text-blue-900">These have been added after the original job pack. Check them before starting and flag anything unclear to the office.</p>
                {controls.customerExtras.length ? (
                  <div className="mt-4 rounded-2xl bg-white p-4 ring-1 ring-inset ring-blue-200">
                    ...
                    <div className="mt-2 space-y-2 text-sm leading-6 text-zinc-800">{controls.customerExtras.map((item, index) => <div key={index}>• {workerSafeText(item)}</div>)}</div>
                  </div>
                ) : null}
                    ...
                    {controls.extraItems.map((item) => (
                      <div key={item.id} className="rounded-2xl bg-white p-4 ring-1 ring-inset ring-blue-200">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="font-black text-zinc-950">{item.type === 'tool' ? '🛠 ' : '📦 '}{workerSafeText(item.item)}</div>
                          <span className={`rounded-full px-3 py-1 text-xs font-black ${item.status === 'on_site' ? 'bg-green-100 text-green-800' : item.status === 'bought' ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-900'}`}>{extraStatus(item.status)}</span>
                        </div>
            ...
            <section className="space-y-3">
              ...
              {plan.dayPlan.map((day) => (
                <article key={day.day} className={`rounded-3xl border p-5 shadow-sm ${day.day === todayDay ? 'border-yellow-300 bg-yellow-50' : 'border-zinc-200 bg-white'}`}>
                  <div className="flex items-start gap-4">
                    <div className="flex h-12 w-12 flex-none items-center justify-center rounded-2xl bg-yellow-300 text-lg font-black">{day.day}</div>
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-black uppercase tracking-wide text-zinc-500">Day {day.day}{day.day === todayDay ? ' — Today' : ''}</div>
                  ...
                  {day.tasks.length ? (
                    <div className="mt-4 space-y-2">{day.tasks.map((task, index) => <div key={`${day.day}-${index}`} className="flex gap-3 rounded-2xl bg-white/80 px-4 py-3 text-sm leading-6 text-zinc-800"><span className="font-black text-zinc-400">□</span><span>{workerTask(task)}</span></div>)}</div>
                  ) : null}
                  <div className="mt-4 rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm leading-6 text-green-950">
                    ...
                    <div className="mt-2 space-y-1">{day.ifAhead.map((task, index) => <div key={`ahead-${day.day}-${index}`}>• {workerTask(task)}</div>)}</div>
                  ...
                  <div className="mt-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-950"><strong>End-of-day checkpoint:</strong> {workerSafeText(day.checkpoint)}</div>
                </article>
              ))}
            </section>
                ...
                <div className="mt-4 space-y-2">
                    ...
                    return (
                      <div key={`${material.item}-${index}`} className="rounded-2xl bg-zinc-50 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          ...
                          <span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${tracking?.status === 'delivered' || tracking?.status === 'stock' ? 'bg-green-100 text-green-800' : tracking?.status === 'ordered' ? 'bg-blue-100 text-blue-800' : 'bg-zinc-200 text-zinc-700'}`}>{materialStatus(tracking?.status)}</span>
                        </div>
                  ...
                  {needsSbr(plan.scope) ? (
                    <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4">
                      ...
                      <div className="mt-1 text-sm leading-6 text-yellow-900">Required in the 1:4 bedding mortar. Make sure SBR/FEB is available and add it in line with the product instructions for the mix being used.</div>
                    </div>
                  ) : null}
                </div>
              </div>
        ...
        <div className="pb-5 text-center text-xs font-medium text-zinc-400">Job #{job.id} · No customer pricing or profit information is shown in the worker job sheet.</div>
      </div>
    </main>
  )
}
