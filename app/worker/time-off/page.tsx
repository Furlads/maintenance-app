...
export default function WorkerTimeOffPage() {
  ...
  async function handleSubmit() {
    ...
    if (!isFullDay && (!startTime || !endTime || endTime <= startTime)) {
      setMessage('Please check the times for this request.')
      return
    }
    ...
    <main style={{ minHeight: '100dvh', background: '#f3f4f6', padding: '16px 0 120px', fontFamily: pageFont, color: '#111827' }}>
      <div style={{ width: '100%', maxWidth: 720, margin: '0 auto', padding: '0 16px', boxSizing: 'border-box' }}>
        <header style={{ background: '#111827', borderRadius: 24, padding: 20, marginBottom: 16 }}>
          <div style={{ color: '#cbd5e1', fontSize: 12, lineHeight: 1.2, textTransform: 'uppercase', fontWeight: 850, letterSpacing: '.08em' }}>
            Worker requests
          </div>
          <h1 style={{ margin: '8px 0 6px', color: '#fff', fontFamily: pageFont, fontSize: 'clamp(30px, 9vw, 42px)', lineHeight: 1, fontWeight: 900, letterSpacing: '-.035em' }}>
            Time Off
          </h1>
          <div style={{ color: '#e5e7eb', fontSize: 16, lineHeight: 1.4, fontWeight: 500 }}>
        ...
        <section style={{ ...cardStyle, marginBottom: 16 }}>
          <h2 style={{ margin: '0 0 16px', fontFamily: pageFont, fontSize: 24, lineHeight: 1.15, fontWeight: 900, letterSpacing: '-.02em' }}>
          ...
          <div style={{ display: 'grid', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: isRangeType(requestType) ? 'repeat(auto-fit, minmax(180px, 1fr))' : '1fr' }}>
              ...
              {isRangeType(requestType) && (
                <div>
                  ...
                  <input id="endDate" type="date" min={startDate} value={endDate} onChange={(e) => setEndDate(e.target.value)} style={inputStyle} />
                </div>
              )}
            </div>
            ...
            {isPartDay(requestType) && (
              <div style={{ padding: 14, borderRadius: 16, border: '1px solid #e5e7eb', background: '#f8fafc' }}>
                ...
                {requestType === 'early_finish' ? (
                  <div>
                    <label htmlFor="finishTime" style={labelStyle}>Finish work at</label>
                    <input id="finishTime" type="time" value={startTime} max={WORK_DAY_END} onChange={(e) => { setStartTime(e.target.value); setEndTime(WORK_DAY_END) }} style={inputStyle} />
                  </div>
                ) : requestType === 'late_start' ? (
                  <div>
                    <label htmlFor="arrivalTime" style={labelStyle}>Start work at</label>
                    <input id="arrivalTime" type="time" value={endTime} min={WORK_DAY_START} onChange={(e) => { setStartTime(WORK_DAY_START); setEndTime(e.target.value) }} style={inputStyle} />
                  </div>
                ) : (
                  <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))' }}>
                    <div>
                      <label htmlFor="startTime" style={labelStyle}>Away from</label>
                      <input id="startTime" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} style={inputStyle} />
                    </div>
                    <div>
                      <label htmlFor="endTime" style={labelStyle}>Back at</label>
            ...
            {(isSingleDayFullDay(requestType) || isRangeType(requestType)) && (
              <div style={{ padding: '11px 13px', borderRadius: 14, background: '#f8fafc', border: '1px solid #e5e7eb', color: '#475569', fontSize: 13, lineHeight: 1.4, fontWeight: 650 }}>
                {requestType === 'holiday' ? 'Holiday is treated as full days. Choose the first and last day you need off.' : 'This request is automatically treated as a full day.'}
              </div>
            )}

            <div>
              <label htmlFor="reason" style={labelStyle}>Reason / notes for Kelly</label>
              <textarea id="reason" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Add any useful notes here" style={{ ...inputStyle, minHeight: 120, resize: 'vertical' }} />
            </div>

            {message && (
              <div style={{ borderRadius: 14, padding: '12px 14px', background: '#f8fafc', border: '1px solid #dbe3ec', color: '#334155', fontSize: 14, lineHeight: 1.4, fontWeight: 750 }}>
                {message}
              </div>
            )}

            <button type="button" onClick={handleSubmit} disabled={busy} style={{ minHeight: 54, borderRadius: 16, border: '1px solid #111827', background: '#111827', color: '#fff', fontFamily: pageFont, fontSize: 16, fontWeight: 900, cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? .65 : 1 }}>
              {busy ? 'Saving...' : editingRequestId ? 'Update request' : 'Send to Kelly'}
            </button>

            {editingRequestId && (
              <button type="button" onClick={resetForm} disabled={busy} style={{ minHeight: 50, borderRadius: 16, border: '1px solid #d1d5db', background: '#fff', color: '#111827', fontFamily: pageFont, fontSize: 16, fontWeight: 850 }}>
                Cancel editing
              </button>
            )}
          </div>
        ...
        <section style={cardStyle}>
          <h2 style={{ margin: '0 0 14px', fontFamily: pageFont, fontSize: 24, lineHeight: 1.15, fontWeight: 900, letterSpacing: '-.02em' }}>My requests</h2>
          {loading ? (
            <div style={{ color: '#64748b', fontWeight: 650 }}>Loading...</div>
          ) : requests.length === 0 ? (
            <div style={{ padding: 16, borderRadius: 16, background: '#f8fafc', border: '1px solid #e5e7eb', color: '#64748b' }}>No requests sent yet.</div>
          ) : (
                  ...
                  <article key={item.id} style={{ padding: 14, borderRadius: 18, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                      <div>
                        ...
                        <div style={{ marginTop: 5, color: '#64748b', fontSize: 14, lineHeight: 1.45, fontWeight: 600 }}>
                          {formatDate(item.startDate)}{item.startDate !== item.endDate ? ` → ${formatDate(item.endDate)}` : ''}{!item.isFullDay && item.startTime && item.endTime ? ` • ${item.startTime}–${item.endTime}` : ' • Full day'}
                        </div>
                      </div>
                      <div style={{ padding: '7px 11px', borderRadius: 999, background: '#fff', border: '1px solid #d1d5db', fontSize: 13, lineHeight: 1, fontWeight: 850, textTransform: 'capitalize' }}>{item.status}</div>
                    </div>
                    ...
                    {pending && (
                      <div style={{ display: 'flex', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
                        <button type="button" onClick={() => startEditing(item)} style={{ minHeight: 42, padding: '0 16px', borderRadius: 12, border: '1px solid #111827', background: '#fff', fontFamily: pageFont, fontWeight: 800 }}>Edit</button>
                        <button type="button" onClick={() => cancelRequest(item.id)} style={{ minHeight: 42, padding: '0 16px', borderRadius: 12, border: '1px solid #fecaca', background: '#fef2f2', color: '#991b1b', fontFamily: pageFont, fontWeight: 800 }}>Cancel</button>
                      </div>
                    )}
                  </article>
                )
