...
type PersistedPhoto = Omit<UploadedPhoto, 'previewUrl'> & {
  previewUrl?: string
}
...
const DRAFT_PREFIX = 'furlads:trev-quote-draft:'
const PHOTO_DB_NAME = 'furlads-quote-drafts'
...
async function savePendingPhoto(jobId: number, photoId: string, file: File) {
  try {
    const db = await openPhotoDb()
    await new Promise<void>((resolve, reject) => {
      const tx = db.transaction(PHOTO_STORE_NAME, 'readwrite')
      tx.objectStore(PHOTO_STORE_NAME).put(file, `${jobId}:${photoId}`)
      tx.oncomplete = () => resolve()
      tx.onerror = () => reject(tx.error)
    })
    db.close()
  } catch {
    // Draft text and already-uploaded photos still recover even if IndexedDB is unavailable.
  }
}

async function loadPendingPhoto(jobId: number, photoId: string) {
  try {
    const db = await openPhotoDb()
    const value = await new Promise<Blob | undefined>((resolve, reject) => {
      const tx = db.transaction(PHOTO_STORE_NAME, 'readonly')
      const request = tx.objectStore(PHOTO_STORE_NAME).get(`${jobId}:${photoId}`)
...
async function deletePendingPhoto(jobId: number, photoId: string) {
  try {
    const db = await openPhotoDb()
    await new Promise<void>((resolve, reject) => {
      const tx = db.transaction(PHOTO_STORE_NAME, 'readwrite')
      tx.objectStore(PHOTO_STORE_NAME).delete(`${jobId}:${photoId}`)
      tx.oncomplete = () => resolve()
      tx.onerror = () => reject(tx.error)
    })
    db.close()
...
async function clearPendingPhotos(jobId: number, photos: PersistedPhoto[]) {
  await Promise.all(photos.map((photo) => deletePendingPhoto(jobId, photo.id)))
}
...
function pricingReply(result: PricingResult) {
  ...
  if (result.optionMode && options.length >= 2) {
    lines.push(
      result.quoteMode === 'packages'
        ? "Right — I’d break this into separate jobs so the customer can see exactly what each part costs."
        : "Right — there are a few different ways we could do this, so I’d show the customer the choices rather than pick for them."
    )
    lines.push('')
    ...
    for (const option of options) {
      ...
      for (const difference of option.keyDifferences || []) lines.push(`• ${difference}`)
      if (option.whyChoose) lines.push(`Why choose it: ${option.whyChoose}`)
      ...
      if (option.estimatedDuration?.workingDays) {
        ...
        lines.push(`Likely install: ${days} ${days === 1 ? 'day' : 'days'} with ${team} ${team === 1 ? 'person' : 'people'}.`)
      }
      lines.push('')
    }
    ...
    if (result.quoteMode === 'packages' && result.combinedOffer?.available) {
      ...
      if (combined.savingExVat > 0) lines.push(`Saving compared with doing the packages separately: ${money(combined.savingExVat)} + VAT`)
      if (combined.savingReason) lines.push(`Why it’s cheaper together: ${combined.savingReason}`)
      if (combined.estimatedDuration?.workingDays) {
        ...
        lines.push(`Likely combined install: ${days} ${days === 1 ? 'day' : 'days'} with ${team} ${team === 1 ? 'person' : 'people'}.`)
      }
      lines.push('')
    }
    ...
    lines.push("If those prices and descriptions look right, send the options to Kelly. The customer can choose after they've seen them.")
    return lines.join('\n')
  }
  ...
  const issues = Array.isArray(result.potentialIssues) ? result.potentialIssues.slice(0, 3) : []
  if (issues.length) {
    lines.push('')
    lines.push('A few things I would keep an eye on:')
    for (const issue of issues) lines.push(`• ${issue.title}: ${issue.details}`)
  }

  const missing = Array.isArray(result.missingInformation) ? result.missingInformation.slice(0, 3) : []
  if (missing.length) {
    lines.push('')
...
export default function AssignedTrevQuotePage() {
  ...
  function makeDraft(): QuoteDraft {
    return {
      ...
      photos: photos.map((photo) => ({
        ...
        status: photo.status,
        previewUrl: photo.uploadedUrl || undefined,
      })),
      pricingResult,
  ...
  function persistLatestDraft() {
    if (!Number.isInteger(jobId) || jobId <= 0 || !latestDraftRef.current || sentToKelly) return
    try {
      localStorage.setItem(draftKey(jobId), JSON.stringify({ ...latestDraftRef.current, updatedAt: Date.now() }))
    } catch {
      // Keep the quote usable even if storage is unavailable.
    }
  }
  ...
  useEffect(() => {
    ...
    async function loadAssignedQuote() {
      try {
        ...
        if (!response.ok || !data) throw new Error(data?.error || 'Could not load this quote visit.')

        const loadedJob = data as Job
        const type = String(loadedJob.jobType || '').toLowerCase()
        const title = String(loadedJob.title || '').toLowerCase()
        if (type !== 'quote' && title !== 'quote') throw new Error('This job is not a quote visit.')
        if (!loadedJob.customer?.id) throw new Error('This quote visit is not linked to a customer.')
        if (cancelled) return

        setJob(loadedJob)
        let restored: QuoteDraft | null = null
        try {
          const raw = localStorage.getItem(draftKey(jobId))
          if (raw) {
            const parsed = JSON.parse(raw) as QuoteDraft
            if (parsed?.version === 1) restored = parsed
          }
        } catch {
          restored = null
        }
        ...
        if (restored) {
          setCustomerReady(restored.customerReady)
          setCustomerName(restored.customerName || loadedJob.customer.name || '')
          setCustomerPhone(restored.customerPhone || loadedJob.customer.phone || '')
          setCustomerEmail(restored.customerEmail || loadedJob.customer.email || '')
          setCustomerAddress(restored.customerAddress || loadedJob.customer.address || loadedJob.address || '')
          setCustomerPostcode(restored.customerPostcode || loadedJob.customer.postcode || '')
          setMessages(Array.isArray(restored.messages) ? restored.messages : [])
          setQuestion(restored.question || '')
          setPricingResult(restored.pricingResult || null)
          if (restored.sessionId) sessionIdRef.current = restored.sessionId

          const restoredPhotos: UploadedPhoto[] = (restored.photos || []).map((photo) => ({
            id: photo.id,
            fileName: photo.fileName,
            uploadedUrl: photo.uploadedUrl || '',
            previewUrl: photo.uploadedUrl || '',
            status: photo.uploadedUrl ? 'uploaded' : photo.status,
          }))
          ...
          for (const photo of restoredPhotos) {
            if (!photo.uploadedUrl) {
              const blob = await loadPendingPhoto(jobId, photo.id)
              if (!blob || cancelled) continue
              const file = new File([blob], photo.fileName, { type: blob.type || 'image/jpeg' })
              const previewUrl = URL.createObjectURL(blob)
              setPhotos((current) =>
                current.map((item) =>
                  item.id === photo.id ? { ...item, previewUrl, status: 'uploading' } : item
                )
              )
              void uploadPhoto(file, photo.id)
            }
          }
        } else {
        ...
        if (!cancelled) {
          setCustomerError(loadError instanceof Error ? loadError.message : 'Could not load this quote visit.')
        }
      } finally {
        if (!cancelled) {
    ...
    if (Number.isInteger(jobId) && jobId > 0) void loadAssignedQuote()
    else {
  ...
  useEffect(() => {
    if (!draftHydrated || !job || sentToKelly) return
    latestDraftRef.current = makeDraft()
    persistLatestDraft()
  }, [
    draftHydrated,
    job,
  ...
  async function handleStartQuote() {
    ...
    try {
      ...
      if (!response.ok || !saved) throw new Error(saved?.error || 'Could not update customer details.')
      ...
      setCustomerError(saveError instanceof Error ? saveError.message : 'Could not update customer details.')
    } finally {
  ...
  async function uploadPhoto(file: File, id: string) {
    ...
    try {
      const response = await fetch('/api/ai/quote/photos', { method: 'POST', body: formData })
      const data = await response.json().catch(() => null)
        ...
        current.map((photo) =>
          photo.id === id
            ? {
                ...
                previewUrl: photo.previewUrl || data.url,
                status: 'uploaded',
              }
            : photo
        )
      )
      void deletePendingPhoto(jobId, id)
    } catch {
      setPhotos((current) =>
        current.map((photo) => (photo.id === id ? { ...photo, status: 'failed' } : photo))
      )
    }
  }
  ...
  async function handlePhotoSelection(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (files.length > remaining) setError('You can keep up to 12 site photos in one quote chat.')

    for (const file of selected) {
      if (!file.type.startsWith('image/') || file.size > 10 * 1024 * 1024) continue
      const id = createId('photo')
      const previewUrl = URL.createObjectURL(file)
      await savePendingPhoto(jobId, id, file)
      setPhotos((current) => [
        ...current,
        { id, fileName: file.name, previewUrl, uploadedUrl: '', status: 'uploading' },
      ])
  ...
  function removePhoto(photoId: string) {
    ...
    void deletePendingPhoto(jobId, photoId)
  }
  ...
  async function handleSend() {
    ...
    const userMessage: ChatMessage = {
      id: createId('trev'),
      role: 'user',
      text: trimmed || `I've added ${uploadedPhotos.length} site ${uploadedPhotos.length === 1 ? 'photo' : 'photos'}. Review them as part of this quote.`,
    ...
    try {
      const response = await fetch('/api/ai/quote', {
        ...
        body: JSON.stringify({
          ...
          photos: uploadedPhotos.map((photo) => ({ url: photo.uploadedUrl, label: 'Site photo' })),
        }),
      })
      const data = await response.json().catch(() => null)
      ...
      setMessages([...nextMessages, { id: createId('chas'), role: 'assistant', text: pricingReply(result) }])
    } catch (sendError) {
      setError(sendError instanceof Error ? sendError.message : 'Something went wrong while I was working the quote out.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function handleSendToKelly() {
    ...
    try {
      ...
      const confirmedScope = [
        ...
        ...(Array.isArray(pricingResult.confirmedInformation) ? pricingResult.confirmedInformation : []),
      ]
        .filter(Boolean)
        .join('\n')
      ...
      const writeResponse = await fetch('/api/ai/quote', {
        ...
        body: JSON.stringify({
          ...
          additionalInstructions: pricingResult.optionMode
      ...
      if (!writeResponse.ok) throw new Error(quoteData?.error || 'I could not write the customer quote.')
      ...
      const sendResponse = await fetch('/api/chas', {
        ...
        body: JSON.stringify({
          ...
          jobId: job.id,
          sessionId: sessionIdRef.current,
          question: pricingResult.optionMode ? 'Options quote draft ready for Kelly review' : 'Quote draft ready for Kelly review',
      ...
      if (!sendResponse.ok || sentData?.success === false) throw new Error('The quote was written, but I could not send it to Kelly.')
      ...
      void clearPendingPhotos(jobId, photos)
    } catch (sendError) {
      setError(sendError instanceof Error ? sendError.message : 'I could not send the quote to Kelly.')
    } finally {
  ...
  if (loading) {
    return <main className="flex min-h-[100dvh] items-center justify-center bg-zinc-100 p-4 text-zinc-700">Loading quote visit…</main>
  }
    ...
    return (
      <main className="min-h-[100dvh] bg-zinc-100 p-4">
        <div className="mx-auto max-w-xl rounded-3xl border border-red-200 bg-white p-5">
          ...
          <Link href="/trev" className="mt-4 inline-flex rounded-xl bg-zinc-950 px-4 py-2 text-sm font-bold text-white">Back to Trev dashboard</Link>
        </div>
      </main>
    )
  }
    ...
    return (
      <main className="min-h-[100dvh] bg-zinc-100 px-3 py-4 text-zinc-950 sm:px-5">
        <div className="mx-auto max-w-3xl">
          ...
          {draftRestored ? <div className="mb-4 rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm font-bold text-green-800">Saved quote draft restored ✓</div> : null}
          ...
          <section className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-600">These have been loaded from Kelly’s quote visit. Correct anything that has changed before starting CHAS.</p>

            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="block sm:col-span-2"><span className="text-sm font-bold">Name *</span><input value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="mt-2 min-h-12 w-full rounded-2xl border border-zinc-300 px-4 text-base outline-none focus:border-zinc-600" /></label>
              <label className="block"><span className="text-sm font-bold">Phone number *</span><input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className="mt-2 min-h-12 w-full rounded-2xl border border-zinc-300 px-4 text-base outline-none focus:border-zinc-600" /></label>
              <label className="block"><span className="text-sm font-bold">Postcode *</span><input value={customerPostcode} onChange={(e) => setCustomerPostcode(e.target.value.toUpperCase())} className="mt-2 min-h-12 w-full rounded-2xl border border-zinc-300 px-4 text-base uppercase outline-none focus:border-zinc-600" /></label>
              <label className="block sm:col-span-2"><span className="text-sm font-bold">Address</span><input value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} className="mt-2 min-h-12 w-full rounded-2xl border border-zinc-300 px-4 text-base outline-none focus:border-zinc-600" /></label>
              <label className="block sm:col-span-2"><span className="text-sm font-bold">Email</span><input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} className="mt-2 min-h-12 w-full rounded-2xl border border-zinc-300 px-4 text-base outline-none focus:border-zinc-600" /></label>
            </div>
            ...
            <button type="button" onClick={handleStartQuote} disabled={customerSaving} className="mt-5 min-h-12 w-full rounded-2xl bg-yellow-300 px-4 py-3 text-sm font-black disabled:opacity-50">{customerSaving ? 'Updating customer…' : 'Confirm details & start quote with CHAS'}</button>
          </section>
        </div>
      </main>
    )
    ...
    <main className="flex h-[100dvh] flex-col overflow-hidden bg-zinc-100 text-zinc-950">
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <button type="button" onClick={() => setCustomerReady(false)} className="block max-w-full truncate text-left text-xs font-bold text-zinc-500 underline decoration-zinc-300 underline-offset-2">{customerName} · {customerPostcode}</button>
            </div>
          </div>
      ...
      <section className="flex-1 overflow-y-auto px-3 py-5 sm:px-5">
        <div className="mx-auto max-w-3xl space-y-4">
          {draftRestored ? <div className="rounded-2xl border border-green-200 bg-green-50 px-4 py-2 text-center text-xs font-bold text-green-800">Saved draft restored ✓</div> : null}
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[88%] whitespace-pre-wrap rounded-3xl px-4 py-3 text-[15px] leading-6 shadow-sm sm:max-w-[82%] ${message.role === 'user' ? 'rounded-br-lg bg-zinc-950 text-white' : 'rounded-bl-lg border border-zinc-200 bg-white text-zinc-900'}`}>{message.text}</div>
            </div>
          ))}
      ...
      <footer className="flex-none border-t border-zinc-200 bg-white px-3 pb-[calc(92px+env(safe-area-inset-bottom))] pt-3 sm:px-5 md:pb-[max(12px,env(safe-area-inset-bottom))]">
        <div className="mx-auto max-w-3xl">
          {photos.length ? (
            <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
              {photos.map((photo) => (
                <div key={photo.id} className="relative h-16 w-16 flex-none overflow-hidden rounded-xl border border-zinc-200 bg-zinc-100">
                  {photo.previewUrl ? <img src={photo.previewUrl} alt={photo.fileName} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-lg">📷</div>}
                  ...
                  <div className={`absolute inset-x-0 bottom-0 py-0.5 text-center text-[9px] font-bold text-white ${photo.status === 'uploaded' ? 'bg-green-700/90' : photo.status === 'failed' ? 'bg-red-700/90' : 'bg-black/70'}`}>{photo.status === 'uploaded' ? 'Ready' : photo.status === 'failed' ? 'Retry on reopen' : 'Uploading'}</div>
                </div>
              ))}
            </div>
          ) : null}

          {pricingResult?.optionMode && !sentToKelly ? <div className="mb-3 rounded-2xl border border-yellow-300 bg-yellow-50 px-4 py-3 text-center text-sm font-bold text-yellow-900">These choices can go to the customer before they decide. Kelly can review and send the full options quote.</div> : null}
          {pricingResult && !sentToKelly ? <button type="button" onClick={handleSendToKelly} disabled={busy || sendingToKelly || uploadsInProgress} className="mb-3 min-h-12 w-full rounded-2xl bg-yellow-300 px-4 py-3 text-sm font-black shadow-sm disabled:opacity-50">{sendingToKelly ? 'Writing it up and sending to Kelly…' : pricingResult.optionMode ? 'Happy with these options — send to Kelly' : 'Happy with this — send to Kelly'}</button> : null}
          ...
          <div className="mt-2 text-center text-[11px] font-medium text-zinc-400">Draft saves automatically ✓ · Enter sends · Shift + Enter adds a new line · up to 12 site photos</div>
        </div>
      </footer>
    </main>
  )
