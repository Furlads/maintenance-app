...
export default function SubcontractorProfilePage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/workers/${worker.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...worker, publicLiabilityExpiresAt: worker.publicLiabilityExpiresAt ? worker.publicLiabilityExpiresAt.slice(0, 10) : '', unavailableUntil: worker.unavailableUntil ? worker.unavailableUntil.slice(0, 10) : '' }) })
      const data = await response.json()
      if (!response.ok) throw new Error(data?.error || 'Could not save subcontractor.')
      setWorker(data.worker); setSaved(true)
    } catch (err) { setError(err instanceof Error ? err.message : 'Could not save subcontractor.') }
  ...
  return <div className="space-y-5 pb-10">
    <div><Link href="/admin/subcontractors" className="text-sm font-black text-zinc-500">← Subcontractors</Link><h1 className="mt-1 text-3xl font-black">{worker.firstName} {worker.lastName}</h1><p className="mt-1 text-sm font-semibold text-zinc-600">Compliance, rates, availability and working arrangements.</p></div>
    ...
    {worker.doNotUse ? <div className="rounded-2xl border border-red-300 bg-red-100 p-4 text-sm font-black text-red-900">Restricted / do not use{worker.doNotUseReason ? ` — ${worker.doNotUseReason}` : ''}</div> : null}

    <form onSubmit={submit} className="space-y-5">
      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
        <h2 className="text-xl font-black">Business, team & rates</h2>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2"><Check label="VAT registered" value={!!worker.vatRegistered} onChange={(value) => setWorker({ ...worker, vatRegistered: value })} /></div>
      </section>

      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
        <h2 className="text-xl font-black">Availability & suitability</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2"><Check label="Drives" value={worker.canDrive} onChange={(value) => setWorker({ ...worker, canDrive: value })} /><Check label="Transport must be arranged" value={worker.transportRequired} onChange={(value) => setWorker({ ...worker, transportRequired: value })} /><Check label="Supplies own tools" value={worker.suppliesTools} onChange={(value) => setWorker({ ...worker, suppliesTools: value })} /><Check label="Can supply materials" value={worker.suppliesMaterials} onChange={(value) => setWorker({ ...worker, suppliesMaterials: value })} /></div>
      </section>

      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black">CIS</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="UTR"><input className="input" value={worker.utrNumber || ''} onChange={(e) => setWorker({ ...worker, utrNumber: e.target.value })} /></Field><Field label="Verification number"><input className="input" value={worker.cisVerificationNumber || ''} onChange={(e) => setWorker({ ...worker, cisVerificationNumber: e.target.value })} /></Field><Field label="Deduction rate %"><input className="input" inputMode="decimal" value={worker.cisDeductionRate ?? ''} onChange={(e) => setWorker({ ...worker, cisDeductionRate: e.target.value === '' ? null : Number(e.target.value) })} /></Field></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><Check label="CIS registered" value={worker.cisRegistered} onChange={(value) => setWorker({ ...worker, cisRegistered: value })} /><Check label="CIS verified" value={worker.cisVerified} onChange={(value) => setWorker({ ...worker, cisVerified: value })} /></div></section>

      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Public liability insurance</h2><div className="mt-4 grid gap-4 sm:grid-cols-3"><Field label="Insurer"><input className="input" value={worker.publicLiabilityInsurer || ''} onChange={(e) => setWorker({ ...worker, publicLiabilityInsurer: e.target.value })} /></Field><Field label="Policy number"><input className="input" value={worker.publicLiabilityPolicyNumber || ''} onChange={(e) => setWorker({ ...worker, publicLiabilityPolicyNumber: e.target.value })} /></Field><Field label="Expiry date"><input type="date" className="input" value={worker.publicLiabilityExpiresAt ? worker.publicLiabilityExpiresAt.slice(0, 10) : ''} onChange={(e) => setWorker({ ...worker, publicLiabilityExpiresAt: e.target.value || null })} /></Field></div></section>

      <section className="rounded-3xl border border-red-200 bg-red-50 p-5"><Check label="Do not offer this subcontractor work" value={!!worker.doNotUse} onChange={(value) => setWorker({ ...worker, doNotUse: value })} /><div className="mt-3"><Field label="Internal reason"><input className="input" value={worker.doNotUseReason || ''} onChange={(e) => setWorker({ ...worker, doNotUseReason: e.target.value })} placeholder="Internal only" /></Field></div></section>
      <section className="rounded-3xl border border-[#d5e3b9] bg-[#f4f8ed] p-5"><Check label="Must accept work before being assigned" value={worker.workAcceptanceRequired} onChange={(value) => setWorker({ ...worker, workAcceptanceRequired: value })} /></section>
    ...
    <style jsx>{`.input{width:100%;border:1px solid #d8dfd3;background:#fff;border-radius:14px;padding:12px 13px;color:#1f2a1b;font:inherit;font-size:14px;font-weight:650;outline:none;box-sizing:border-box}.input:focus{border-color:#9fbe55;box-shadow:0 0 0 3px rgba(159,190,85,.14)}`}</style>
  </div>
}

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-2 block text-[11px] font-black uppercase tracking-wider text-zinc-500">{label}</span>{children}</label> }
function Check({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) { return <label className="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-4 text-sm font-black"><input type="checkbox" checked={value} onChange={(e) => onChange(e.target.checked)} className="h-5 w-5" />{label}</label> }
