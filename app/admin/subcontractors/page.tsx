    ...
    <div className="sub-page">
      <style>{`
        ...
        .sub-hero {
          ...
          padding: 30px;
          color: white;
          background: linear-gradient(135deg,#152315 0%,#273c1d 65%,#3b5625 100%);
          box-shadow: 0 22px 55px rgba(24,45,17,.17);
        }
        .sub-hero:after {
          content: '';
          position: absolute;
        ...
        @media (max-width:900px) {
          ...
          .sub-opportunity { grid-template-columns:1fr 1fr; }
        }
        @media (max-width:620px) {
          .sub-hero { padding:22px; border-radius:23px; }
          .sub-title { font-size:38px; }
          .sub-stats { grid-template-columns:1fr 1fr; gap:8px; }
          .sub-stat { padding:15px; border-radius:17px; }
          .sub-stat-value { font-size:22px; }
          .sub-section { padding:17px; border-radius:20px; }
          .sub-section-head { align-items:flex-start; }
          .sub-opportunity { grid-template-columns:1fr; gap:11px; }
          .sub-actions .sub-btn { width:100%; }
        }
      `}</style>
      ...
      <section className="sub-hero">
        ...
        <p className="sub-copy">
          Send work out without exposing customer details, agree a trade price or request a quote, and keep every response attached to the job.
        </p>
      ...
      <section className="sub-section">
        <div className="sub-section-head">
          <div><div className="sub-section-kicker">Live work</div><h2 className="sub-section-title">Current opportunities</h2></div>
          <Link className="sub-section-link" href="/admin/subcontractors/new">Send another →</Link>
        </div>
        <div className="sub-list">
          {opportunities.map((opportunity) => (
            <div className="sub-opportunity" key={opportunity.id}>
              <div>
                <div className="sub-id">{opportunity.id}</div>
                <div className="sub-job">{opportunity.title}</div>
                <div className="sub-meta">{opportunity.trade} · {opportunity.area}</div>
              </div>
      ...
      <div className="sub-two-col">
        ...
        <section className="sub-section">
          ...
          <div className="sub-note">
            <strong>Initial opportunity:</strong> trade, rough area, plain-English job description, likely duration and either a proposed subcontract price or “quote requested”.
            <br /><br />
            Customer name, exact address, phone number, email and identifying photos stay hidden until the contractor asks for more details and you choose to release them.
          </div>
        </section>
      </div>
    </div>
