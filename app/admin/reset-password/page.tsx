...
export default function ResetPasswordPage() {
  ...
  const requestedWorkerId = Number(searchParams.get("workerId"));
  ...
  useEffect(() => {
    if (Number.isInteger(requestedWorkerId) && requestedWorkerId > 0) {
      setSelectedWorkerId(requestedWorkerId);
    }
  }, [requestedWorkerId]);
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    try {
      ...
      setPassword("");
      setMessage(
        `✅ Password reset for ${selectedWorker?.fullName || "worker"}`
      );
    } catch (error: any) {
