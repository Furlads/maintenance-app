...
export async function POST(req: Request) {
  try {
    ...
    const startDateText = normaliseDateText(
      safeString(body.startDate)
    )

    const endDateText = normaliseDateText(
      safeString(body.endDate)
    )
    ...
    const requestedByName =
      safeString(body.requestedByName) || null
    ...
    if (!startDateText || !endDateText) {
      return NextResponse.json(
        { error: 'Missing or invalid start/end date' },
        { status: 400 }
      )
    }

    const startDate = new Date(
      `${startDateText}T00:00:00.000Z`
    )

    const endDate = new Date(
      `${endDateText}T00:00:00.000Z`
    )
    ...
    if (!isFullDay) {
      if (!startTime || !endTime) {
        return NextResponse.json(
          {
            error:
              'Start and end times are required for part-day requests',
          },
          { status: 400 }
        )
      }
    ...
    console.error(
      'POST /api/time-off/request failed',
      error
    )
