...
async function syncAcceptedJobAssignments(jobId: number) {
  const acceptedRecipients = await prisma.$queryRaw<AcceptedRecipient[]>`
    ...
    JOIN "SubcontractorOpportunity" o ON o."id" = r."opportunityId"
    JOIN "Worker" w ON w."id" = r."workerId"
    WHERE o."sourceJobId" = ${jobId} AND r."status" = 'accepted' AND w."active" = TRUE
  `
  const acceptedWorkerIds = new Set(acceptedRecipients.map((item) => item.workerId))
  const acceptanceRequiredWorkers = await prisma.worker.findMany({ where: { workAcceptanceRequired: true }, select: { id: true } })
  const acceptanceRequiredIds = new Set(acceptanceRequiredWorkers.map((item) => item.id))
  await prisma.jobAssignment.deleteMany({ where: { jobId, workerId: { in: [...acceptanceRequiredIds].filter((id) => !acceptedWorkerIds.has(id)) } } })

  const existingAssignments = await prisma.jobAssignment.findMany({ where: { jobId }, include: { worker: { select: { id: true, canDrive: true } } } })
  ...
  for (const recipient of acceptedRecipients.filter((item) => !item.transportRequired)) {
  ...
  for (const recipient of acceptedRecipients.filter((item) => item.transportRequired)) {
    if (!assignedIds.has(recipient.workerId) && hasDriver) {
      await prisma.jobAssignment.create({ data: { jobId, workerId: recipient.workerId } })
      assignedIds.add(recipient.workerId)
    }
  }
...
async function authorisedOpportunity(token: string) {
...
export async function GET(_: Request, ctx: Ctx) {
  ...
  const auth = await authorisedOpportunity(token)
  if (auth.error || !auth.opportunity) return auth.error!
  const opportunity = auth.opportunity

  await prisma.$executeRaw`UPDATE "SubcontractorOpportunityRecipient" SET "viewedAt" = COALESCE("viewedAt", CURRENT_TIMESTAMP), "status" = CASE WHEN "status" = 'sent' THEN 'viewed' ELSE "status" END WHERE "id" = ${opportunity.recipientId}`
  ...
  if (opportunity.status === 'accepted' && opportunity.sourceJobId) {
    const assignment = await prisma.jobAssignment.findFirst({ where: { jobId: opportunity.sourceJobId, workerId: opportunity.workerId }, select: { id: true } })
    assignmentStatus = assignment ? 'confirmed' : opportunity.transportRequired ? 'transport_required' : 'confirmed'
  } else if (opportunity.status === 'accepted') assignmentStatus = 'not_linked'
