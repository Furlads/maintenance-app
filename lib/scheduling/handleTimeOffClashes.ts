...
export async function handleTimeOffClashes({
  ...
  for (const job of conflictingJobs) {
    ...
    if (isQuoteJob(job)) {
      ...
      if (nextSlot) {
        ...
        await addJobAuditNote(
          job.id,
          `System auto-moved quote after time off clash handling. ${detail}`
        )
        ...
        await createKellyAlert({
          ...
          preview: detail,
          body: [
            `${requestedByName} triggered time off clash handling for worker ${workerId}.`,
            `Type: ${requestType}.`,
    ...
    const isSoloWorkerJob =
      ...
      job.assignments[0]?.workerId === workerId
    ...
    if (isSoloWorkerJob) {
      ...
      if (replacementWorker) {
        ...
        await addJobAuditNote(
          job.id,
          `System reassigned job after time off clash handling. ${detail}`
        )
    ...
    await addJobAuditNote(
      job.id,
      `Time off clash detected. ${detail}`
    )
