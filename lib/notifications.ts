...
function getLondonOffsetMinutes(date: Date) {
...
function parseLondonDateTimeToUtc(date: Date, hhmm: string) {
  ...
  const offsetMinutes = getLondonOffsetMinutes(utcBase)
...
async function getPreviousTrevStopPostcode(job: JobWithAlertData) {
  ...
  const previousJob = await prisma.job.findFirst({
    where: {
      id: { not: job.id },
      visitDate: {
        gte: startOfLondonDayUtc(job.visitDate),
        lt: nextLondonDayUtc(job.visitDate),
      },
      status: {
...
export async function syncJobAlerts(jobId: number) {
  ...
  const records = phones.flatMap((phone) => {
    ...
    items.push({
      ...
      status: oneHourBeforeUtc <= now ? CANCELLED_STATUS : PENDING_STATUS,
      failureReason: oneHourBeforeUtc <= now ? 'Skipped because scheduled time is already in the past' : null,
    })
    ...
    items.push({
      ...
      status: leaveNowUtc <= now ? CANCELLED_STATUS : PENDING_STATUS,
      failureReason: leaveNowUtc <= now ? 'Skipped because scheduled time is already in the past' : null,
    })
